...
export default async function QAUsersPage({ 
  searchParams 
}: { 
  ...
  // চেক করা হচ্ছে লগিন করা ব্যক্তি QA কি না
  const { data: profile } = await supabase.from('profiles').select('role').eq('id', user.id).single()
  if (profile?.role !== 'qa') redirect('/dashboard')

  const supabaseAdmin = createAdminClient()
  
  // শুধুমাত্র 'agent' রোলের ইউজারদের আনা হচ্ছে
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1 bg-white p-6 rounded-2xl shadow-sm border border-gray-100 h-fit">
            ...
            {searchParams.success && <div className="mb-4 p-3 bg-green-50 text-green-700 text-sm rounded-lg font-poppins">{searchParams.success}</div>}
            {searchParams.error && <div className="mb-4 p-3 bg-red-50 text-red-700 text-sm rounded-lg font-poppins">{searchParams.error}</div>}
            ...
            <form action={createAgent} className="space-y-4">
              ...
              {/* Role Fixed to Agent, user cannot change it */}
                  ...
                  {agents?.length === 0 ? (
                    ...
                    agents?.map((u) => (
                      <tr key={u.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <span className="px-3 py-1 rounded-full text-xs font-medium bg-shikho-indigo-50 text-shikho-indigo-600 font-poppins">
                            AGENT
                          </span>
                        </td>
                        <td className="px-6 py-4 text-right">
                          <form action={deleteAgent}>
                            ...
                            <button type="submit" className="text-xs text-red-600 bg-red-50 px-4 py-2 rounded-lg hover:bg-red-100 font-bold font-poppins transition-colors">
                              Delete Agent
                            </button>
                          </form>
                        </td>
